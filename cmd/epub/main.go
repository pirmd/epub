...
func printField(metadata *epub.Information, field string) {
	...
	case "collections":
		for _, col := range metadata.Collections {
			fmt.Printf("Collection: %s (role: %s, position: %d)\n", col.Title, col.Role, col.Position)
		}
	default:
...
func main() {
	...
	flag.StringVar(&fields, "field", "", "Print only specific field(s). Comma-separated list: title,author,language,identifier,publisher,date,description,series,epubVersion,collections,etc.")
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(flag.CommandLine.Output(), "  coverage, relation, epubVersion, collections\n")
		fmt.Fprintf(flag.CommandLine.Output(), "\n")
		...
		fmt.Fprintf(flag.CommandLine.Output(), "  %s --field epubVersion book.epub  # Print EPUB version\n", filepath.Base(os.Args[0]))
		fmt.Fprintf(flag.CommandLine.Output(), "  %s --field collections book.epub  # Print EPUB3 collections\n", filepath.Base(os.Args[0]))
